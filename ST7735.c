...
void Delay1ms(uint32_t n){uint32_t volatile time;
  while(n){
    time = 72724*4/91;  // 1msec, tuned at 80 MHz
    while(time){
      time--;
    }
    n--;
...
uint32_t ST7735_DrawString(uint16_t x, uint16_t y, char *pt, int16_t textColor, int16_t size ){
  uint32_t count = 0;
  if(y>15) return 0;
  while(*pt){
...
void ST7735_OutUDec(uint32_t n){
  ...
  ST7735_DrawString(StX,StY,Message,StTextColor,1);
...
void ST7735_OutChar(char ch){
  if((ch == 10) || (ch == 13) || (ch == 27)){
    ...
    ST7735_DrawString(0,StY,"                     ",StTextColor, 1);
    return;
  }
